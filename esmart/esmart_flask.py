...
# -*- coding: utf-8 -*-
#
#  esmart_flask.py - Flask web server for eSmart, for visualizing data,
...
from databases.esmart_db.models import DisplayOrder
from databases.esmart_db.models import Graph
from databases.esmart_db.models import LCD
from databases.esmart_db.models import Log
from databases.esmart_db.models import Method
from databases.esmart_db.models import Misc
from databases.esmart_db.models import PID
from databases.esmart_db.models import Relay
from databases.esmart_db.models import RelayConditional
from databases.esmart_db.models import Remote
from databases.esmart_db.models import Sensor
from databases.esmart_db.models import SensorConditional
from databases.esmart_db.models import SMTP
from databases.esmart_db.models import Timer
...
from config import ESMART_VERSION
...
misc = flaskutils.db_retrieve_table(ESMART_DB_PATH, Misc, first=True)
if misc.force_https:
    sslify = SSLify(app)
...
@app.before_request
def before_request():
    if (not os.path.isfile(SQL_DATABASE_ESMART) or
...
def page(page):
    ...
    if page == 'live':
        ...
        pid = flaskutils.db_retrieve_table(ESMART_DB_PATH, PID)
        relay = flaskutils.db_retrieve_table(ESMART_DB_PATH, Relay)
        sensor = flaskutils.db_retrieve_table(ESMART_DB_PATH, Sensor)
        timer = flaskutils.db_retrieve_table(ESMART_DB_PATH, Timer)

        pid_display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).pid
        if pid_display_order_unsplit:
            pid_display_order = pid_display_order_unsplit.split(",")
        else:
            pid_display_order = []

        sensor_display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).sensor
        ...
        display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).graph
        if display_order_unsplit:
            display_order = display_order_unsplit.split(",")
        else:
            display_order = []
        graph = flaskutils.db_retrieve_table(ESMART_DB_PATH, Graph)
        pid = flaskutils.db_retrieve_table(ESMART_DB_PATH, PID)
        relay = flaskutils.db_retrieve_table(ESMART_DB_PATH, Relay)
        sensor = flaskutils.db_retrieve_table(ESMART_DB_PATH, Sensor)
    ...
    # Modify sensor SQL database
        ...
        display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).sensor
        if display_order_unsplit:
            display_order = display_order_unsplit.split(",")
        else:
            display_order = []
        lcd = flaskutils.db_retrieve_table(ESMART_DB_PATH, LCD)
        pid = flaskutils.db_retrieve_table(ESMART_DB_PATH, PID)
        relay = flaskutils.db_retrieve_table(ESMART_DB_PATH, Relay)
        sensor = flaskutils.db_retrieve_table(ESMART_DB_PATH, Sensor)
        sensor_conditional = flaskutils.db_retrieve_table(ESMART_DB_PATH, SensorConditional)
        ...
        sensor_path = "{}/esmart/frontend/templates/pages/sensor_options/".format(INSTALL_DIRECTORY)
        ...
        display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).relay
        if display_order_unsplit:
            display_order = display_order_unsplit.split(",")
        else:
            display_order = []
        lcd = flaskutils.db_retrieve_table(ESMART_DB_PATH, LCD)
        relay = flaskutils.db_retrieve_table(ESMART_DB_PATH, Relay)
        relayconditional = flaskutils.db_retrieve_table(ESMART_DB_PATH, RelayConditional)
        users = flaskutils.db_retrieve_table(USER_DB_PATH, Users)
        ...
        display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).pid
        if display_order_unsplit:
            display_order = display_order_unsplit.split(",")
        else:
            display_order = []

        pids = flaskutils.db_retrieve_table(ESMART_DB_PATH, PID)
        relay = flaskutils.db_retrieve_table(ESMART_DB_PATH, Relay)
        sensor = flaskutils.db_retrieve_table(ESMART_DB_PATH, Sensor)
        ...
        display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).lcd
        if display_order_unsplit:
            display_order = display_order_unsplit.split(",")
        else:
            display_order = []
        ...
        display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).log
        if display_order_unsplit:
            display_order = display_order_unsplit.split(",")
        else:
            display_order = []
        log = flaskutils.db_retrieve_table(ESMART_DB_PATH, Log)
        sensor = flaskutils.db_retrieve_table(ESMART_DB_PATH, Sensor)
        ...
        display_order_unsplit = flaskutils.db_retrieve_table(ESMART_DB_PATH, DisplayOrder, first=True).timer
        if display_order_unsplit:
            display_order = display_order_unsplit.split(",")
        else:
            display_order = []
        ...
        backup_dirs = []
        backups_sorted = []
        if not os.path.isdir('/var/eSmart-backups'):
            flash("Error: Backup directory doesn't exist.", "error")
        else:
            backup_dirs = sorted(next(os.walk('/var/eSmart-backups'))[1])
        ...
        if request.method == 'POST':
            ...
            if form_name == 'restore':
                if formBackup.restore.data:
                    ...
                    # restore_command = INSTALL_DIRECTORY+'/esmart/scripts/esmart_wrapper restore '+ +'  >> /var/log/esmart/esmartrestore.log 2>&1'
        ...
        cmd_output("git fetch origin", su_esmart=False)
        current_commit, _, _ = cmd_output("git rev-parse --short HEAD", su_esmart=False)
        commits_behind, _, _ = cmd_output("git log --oneline | head -n 1", su_esmart=False)
        commits_behind_list = commits_behind.split('\n')
        commits_ahead, commits_ahead_err, _ = cmd_output("git log --oneline master...origin/master", su_esmart=False)
        ...
        if request.method == 'POST':
            ...
            if form_name == 'update':
                if formUpdate.update.data:
                    if update_available:
                        subprocess.Popen(INSTALL_DIRECTORY + '/esmart/scripts/esmart_wrapper upgrade >> /var/log/esmart/esmartupdate.log 2>&1', shell=True)
    ...
    # Display relay usage
        ...
        display_order_unsplit = flaskutils.db_retrieve_table(
            ESMART_DB_PATH, DisplayOrder, first=True).relay
        if display_order_unsplit:
            display_order = display_order_unsplit.split(",")
        else:
            display_order = []
            ...
            if form_name == 'camera':
                if formCamera.Still.data:
                    ...
                    if not stream_locked:
                        ...
                        uid_gid = pwd.getpwnam('esmart').pw_uid
...
def method_builder(method_type, method_id):
    ...
    if method_type in ['Date', 'Duration', 'Daily', 'DailySine', '0']:
        ...
        # method = flaskutils.db_retrieve_table(ESMART_DB_PATH, Method)
        relay = flaskutils.db_retrieve_table(ESMART_DB_PATH, Relay)
...
def remote_admin(page):
    ...
    remote_hosts = flaskutils.db_retrieve_table(ESMART_DB_PATH, Remote)
    display_order_unsplit = flaskutils.db_retrieve_table(
        ESMART_DB_PATH, DisplayOrder, first=True).remote_host
    if display_order_unsplit:
        display_order = display_order_unsplit.split(",")
    else:
        display_order = []
...
def settings(page):
    ...
    # Alert email notifification settings
        ...
        smtp = flaskutils.db_retrieve_table(ESMART_DB_PATH, SMTP)
        formEmailAlert = flaskforms.EmailAlert()
        if request.method == 'POST':
            form_name = request.form['form-name']
            # Update smtp settings table in esmart SQL database
    ...
    # General settings
        ...
        misc = flaskutils.db_retrieve_table(ESMART_DB_PATH, Misc)
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="eSmart Flask HTTP server.",
