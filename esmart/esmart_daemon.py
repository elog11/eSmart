...
# -*- coding: utf-8 -*-
#
#  esmart_daemon.py - Daemon for managing eSmart controllers, such as sensors,
...
class ComServer(rpyc.Service):
    """
    Class to handle communication between the client (esmart_client.py) and
    the daemon (esmart_daemon.py). This also serves as how other controllers
    ...
    def exposed_terminate_daemon(self):
        ...
        return esmart_daemon.terminateDaemon()
...
class DaemonController(threading.Thread):
    """
    Class for running the eSmart daemon
    ...
    def run(self):
        ...
        self.logger.info("[Daemon] eSmart terminated in {} seconds".format(
    ...
    def send_stats(self):
        ...
        try:
            ...
            os.remove(STATS_CSV)
            recreate_stat_file(ID_FILE, STATS_CSV, STATS_INTERVAL, ESMART_VERSION)
        try:
            send_stats(self.logger, STATS_HOST, STATS_PORT,
                                   STATS_USER, STATS_PASSWORD, STATS_DATABASE,
                                   ESMART_DB_PATH, USER_DB_PATH,
                                   STATS_CSV, ESMART_VERSION,
    ...
    def startup_stats(self):
        ...
        try:
            ...
            self.logger.info("[Daemon] eSmart v{} started in {} seconds".format(
                ESMART_VERSION, daemon_startup_time))
            add_update_csv(self.logger, STATS_CSV,
...
def start_daemon():
    ...
    ct = ComThread()
    ct.daemon = True
    # Start communication thread for receiving commands from esmart_client.py
    ct.start()
...
def parse_args():
    parser = argparse.ArgumentParser(description='eSmart daemon.')
...
if __name__ == '__main__':
    ...
    global esmart_daemon
    esmart_daemon = DaemonController(logger)

    # Set up daemon and start it
    daemon = Daemonize(app="eSmartd",
                       pid=DAEMON_PID_FILE,
                       action=start_daemon,
                       keep_fds=keep_fds)
    daemon.start()
